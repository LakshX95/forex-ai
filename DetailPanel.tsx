...
import { fmtPrice, fmtPips, pnlColor } from '../lib/utils'
    ...
    <div className="card mt-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <LevelRow label="FVG" value={signal.fvg_detected ? `Active (${signal.fvg_direction})` : 'None'} color={signal.fvg_detected ? 'text-sky-400' : 'text-zinc-600'} />
          <LevelRow label="Order Block" value={signal.order_block_hit ? 'Hit' : 'Clear'} color={signal.order_block_hit ? 'text-amber-400' : 'text-zinc-600'} />
          <LevelRow label="Kill Zone" value={`${signal.kill_zone}${signal.kill_zone_active ? ' ●' : ''}`} color={signal.kill_zone_active ? 'text-emerald-400' : 'text-zinc-500'} />
        </div>
      </div>
